...
def _apply_single_edit(samples: np.ndarray, sample_rate: int, t1: float, t2: float, scale: float) -> np.ndarray:
    if scale <= 0:
...
def process_file(input_path: str, output_path: str, t1: float, t2: float, scale: float):
    ...
    output = _apply_single_edit(samples, sample_rate, t1, t2, scale)
